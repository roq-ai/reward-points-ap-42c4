...
import { MerchantListPage as ShowCaseList } from 'pages/merchants';
...
function DashboardPage() {
  ...
  const merchantStat = useSWR<PaginatedInterface<MerchantInterface>>(
    () => `/merchants`,
    () => getMerchants({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Merchants',
      result: merchantStat?.data?.totalCount,
      isLoading: merchantStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Transactions',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`merchants`} passHref legacyBehavior>
                  <Button
